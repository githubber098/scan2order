...
_VLM_PROMPT = (
    ...
    "Transcribe EVERY item, exactly once, ONE item per line, in the order written. "
    "Do not skip any line, and do not repeat a line. "
    "Use common grocery knowledge to fix obvious misreads to the most likely real "
    "product — for example a line that looks like 'Green Yogurt' is almost certainly "
    "'Greek Yogurt'. Only correct clear misreads; NEVER invent items that are not "
    "written on the list. "
    "IMPORTANT: Unit-of-measure words (Pint, Quart, Litre, Ounce, Pound, Dozen) are "
    "NOT grocery items — do not output them as standalone items. If a word looks like "
    "a unit of measure, it is probably a misread of an actual grocery item nearby. "
    "If multiple items are grouped on one line separated by a dash ( - ), hyphen, or "
    "plus ( + ), write each as its OWN separate line "
...
_CORRECTION_PROMPT = (
    "You are cleaning up a grocery shopping list that was transcribed from a photo "
    "by OCR, so some lines may contain reading errors. For each line, output the most "
    "likely INTENDED grocery item, using common product knowledge to fix misreads "
    "(for example 'Green Yogurt' → 'Greek Yogurt'). "
    "Keep each line's quantity and unit unchanged (e.g. '500 gm', '1 kg'). "
    "IMPORTANT: Remove any line that is a bare unit-of-measure word with no product "
    "context — words like 'Pint', 'Quart', 'Litre', 'Ounce', 'Pound', 'Dozen' alone "
    "are not grocery items and were almost certainly an OCR misread that could not be "
    "recovered. Do not guess what they were; simply drop them. "
    "Remove a line ONLY when it is an exact duplicate OR a bare unit-of-measure word "
    "as described above. "
    "Do NOT add new items, and do NOT drop distinct valid grocery items. "
